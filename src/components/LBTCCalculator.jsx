...
const LBTCCalculator = () => {
  ...
  const sendIconBgClass = isSwapped 
    ? "bg-linear-to-b from-[#2D5F5D] to-[#3A7875]" 
    : "bg-linear-to-b from-[#FF8904] to-[#F54900]";

  const receiveIconBgClass = isSwapped 
    ? "bg-linear-to-b from-[#FF8904] to-[#F54900]" 
    : "bg-linear-to-b from-[#2D5F5D] to-[#3A7875]";

  const receiveBorderClass = isSwapped 
    ? "" 
    : "border border-[#2D5F5D]/50 bg-[#3A7875]/10";

  const sendBorderClass = isSwapped 
    ? "border border-[#2D5F5D]/50 bg-[#3A7875]/10" 
    : "";
    ...
    <div className="text-center py-35 flex flex-col items-center bg-white">
      ...
      <div className="bg-linear-to-tl from-[#2D5F5D]/10 via-white to-[#2D5F5D]/10 p-10 rounded-4xl mt-16 shadow-[0_20px_60px_rgba(0,0,0,0.25)]">
        ...
        <div className="flex flex-row items-center gap-2 mt-3">
          ...
          />
          <div className={`text-[12px] shadow-lg rounded-3xl inset-shadow-sm flex flex-row items-center justify-center w-[160px] h-[78px] gap-4 ${sendBorderClass}`}>
            <div className={`w-[40px] h-[40px] ${sendIconBgClass} rounded-4xl shadow-lg flex items-center justify-center`}>
              <SendIcon />
            </div>
            <div className="min-w-8 text-[20px]">{sendCurrency}</div>
          </div>
        </div>
        <motion.div 
          onClick={handleSwap}
          ...
          className="bg-green-300 justify-self-center bg-linear-to-b from-[#2D5F5D] to-[#3A7875] w-[55px] h-[55px] mt-9 rounded-xl shadow-lg flex items-center justify-center cursor-pointer"
        >
